...
<div class="tab">
 ...
 <el-divider class="boderline"/>
    ...
    <el-table :data="tableData" border="true" style= border :header-cell-style="{'text-align':'center'}"  :cell-style="{'text-align':'center'}" ref="tableN">
    <el-table-column prop="id" label="序号" width="150" />
    <el-table-column prop="name" label="消息体名称" width="250" />
    <el-table-column prop="type" label="消息体类型" width="150" />
  ...
  <div class="pagination-block">
    ...
    <MessageAdd ref="testDialog" @addValue='addData' @changeValue="editData"></MessageAdd>
</div>
</div>
</template>
...
import MessageAdd from "@/components/networktable/MessageAddDialogForm";
...
const  handleClickEdit=(index, row,flag)=>{
      ...
      router.push('/MessageDetailTable?id='+row.id+'&name='+row.name+'&type='+row.type)
    }
      }
...
const getList=()=>{
    HTTPRequest.post('/message/list').then(res=>{
    tableData.value=res.data;
...
const editData = (fo) => {
    ...
    let index = tableData.value.findIndex(
          ...
          HTTPRequest.post('/message/update',
        {
           'id':fo.id,
            'name':fo.name,
            'type':fo.type
...
const  handleClickDelete=(i, row)=>{
    ...
    let index = tableData.value.findIndex(
       ...
       HTTPRequest.post('/message/delete',
        {
            id:row.id
        }).then(res=>{
            tableData.value.splice(index, 1);
